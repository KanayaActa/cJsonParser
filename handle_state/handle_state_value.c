...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   handle_state_value.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: miwasa <miwasa@student.42.fr>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/02/24 23:00:20 by miwasa            #+#    #+#             */
/*   Updated: 2025/02/25 13:15:00 by miwasa           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_value_left_brace(t_json_parser_stack *ps)
{
	...
	objv = json_value_new(JSON_OBJECT);
	objv->u_.object_value = ft_calloc(1, sizeof(t_json_object));
	push_value(ps, objv);
	pop_state(ps);
	push_state(ps, STATE_EOF);
...
void	parse_value_left_bracket(t_json_parser_stack *ps)
{
	...
	arrv = json_value_new(JSON_ARRAY);
	arrv->u_.array_value = ft_calloc(1, sizeof(t_json_array));
	push_value(ps, arrv);
	pop_state(ps);
	push_state(ps, STATE_EOF);
...
void	parse_value_string(t_json_parser_stack *ps)
{
	...
	strv = json_value_new(JSON_STRING);
	strv->u_.string_value = ft_strdup(ps->current.text);
	push_value(ps, strv);
	pop_state(ps);
